...
<div class="grid grid-cols-1 gap-8">
    ...
    <div class="flex justify-between items-center p-8 rounded-[2rem] shadow-neumorphic bg-neu-base">
        <div>
            <h3 class="text-xl font-bold text-gray-800">Total Lessons: {{ $lessons->count() }}</h3>
            <p class="text-sm text-gray-500">Manage your course content, uploads, and assignments.</p>
        </div>
        <a href="{{ route('admin.lessons.create') }}" class="px-8 py-3 rounded-2xl shadow-neumorphic-btn hover:shadow-neumorphic-btn-active font-bold text-primary transition duration-200">
            Create New Lesson
        </a>
    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition duration-150">
                        ...
                        <td class="px-6 py-5">
                            <div class="flex gap-2">
                                ...
                                @endif
                                @if(!$lesson->pdf_path && !$lesson->video_path && !$lesson->image_path)
                        ...
                        <td class="px-6 py-5 text-right flex justify-end gap-3">
                            <!-- Edit Button -->
                            <a href="{{ route('admin.lessons.edit', $lesson->id) }}" class="w-10 h-10 rounded-xl bg-brand-50 text-brand-600 hover:bg-brand-600 hover:text-white transition-all duration-300 flex items-center justify-center">
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-10 text-center text-gray-500 italic rounded-xl shadow-neumorphic-inset mt-4">
                            No lessons found. Click "Create New Lesson" to get started.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
