...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6 h-fit">
        <h3 class="text-base font-semibold text-gray-900 mb-6 border-l-4 border-indigo-600 pl-3">New Institute</h3>
        <form action="{{ route('admin.schools.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <x-input-label for="name" value="Official Name" />
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="text-gray-500 text-xs uppercase tracking-wide font-medium border-b border-gray-100 bg-gray-50">
                        ...
                        <th class="px-6 py-3">Established</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 font-bold text-gray-900">{{ $school->name }}</td>
                        <td class="px-6 py-4 text-xs text-gray-500">{{ $school->created_at->format('M d, Y') }}</td>
                        <td class="px-6 py-4 text-right flex justify-end gap-2">
                            <!-- Edit Button -->
                            <button onclick="openEditModal({{ json_encode($school) }})" class="w-9 h-9 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100 flex items-center justify-center border border-indigo-200 transition">
                            ...
                            <form action="{{ route('admin.schools.destroy', $school->id) }}" method="POST" onsubmit="return confirm('Archive institution record?')" class="inline">
                                @csrf
                                @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="3" class="px-6 py-12 text-center text-gray-400 italic">No institutions found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<!-- Edit School Modal -->
<div id="edit-school-modal" class="hidden fixed inset-0 z-50 items-center justify-center p-4 bg-black/50">
    <div class="bg-white p-6 rounded-lg border border-gray-200 max-w-lg w-full">
        <div class="flex justify-between items-center mb-6">
            <h4 class="text-lg font-bold text-gray-900">Modify School</h4>
            <button onclick="document.getElementById('edit-school-modal').classList.add('hidden')" class="text-gray-400 hover:text-gray-600 transition-colors">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>
        </div>
        <form id="edit-school-form" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
            ...
            <div class="flex gap-3 pt-4">
                <button type="button" onclick="document.getElementById('edit-school-modal').classList.add('hidden')" class="flex-1 py-2.5 rounded-lg bg-gray-100 text-gray-600 font-bold hover:bg-gray-200 transition-colors text-sm">Cancel</button>
